...
**Options:** * `--provider, -p TEXT`: Filter by provider (e.g., `openai`, `anthropic`). * `--format, -f [text|json]`: Output format.
...
**Options:** * `--force, -f`: Force refresh even if cache is fresh (< 24h). * `--format [text|json]`: Output format.
...
**Options:** * `--period [week|month|quarter|all]`: Time period to analyze. Default is `month`. * `--db PATH`: Path to SQLite database. Default: `./output/watcher.db`. * `--format, -f [text|json]`: Output format.
...
**Options:** * `--output, -o PATH` **(Required)**: Output file path. Extension (`.csv` or `.json`) determines format. * `--db PATH`: Path to SQLite database. Default: `./output/watcher.db`. * `--run-id TEXT`: Filter by specific run ID. * `--days INTEGER`: Include only last N days of data.
...
**Options:** * `--output, -o PATH` **(Required)**: Output file path. Extension (`.csv` or `.json`) determines format. * `--db PATH`: Path to SQLite database. Default: `./output/watcher.db`. * `--days INTEGER`: Include only last N days of data.
